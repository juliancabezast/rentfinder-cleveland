-- Fix: the Emails hero card's "en cola" was reading comms.queue_pending, which
-- counts pending AGENT TASKS, not queued emails. Add comms.email_queue (a real
-- count of email_events awaiting send) so the card reflects the email pipeline.
-- Full function body kept identical except the added 'email_queue' field.
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'comms', json_build_object(
        ...
        'email_queue', (SELECT COUNT(*) FROM email_events
          WHERE organization_id = _org AND event_type = 'delivery_delayed' AND details->>'status' = 'queued'),
