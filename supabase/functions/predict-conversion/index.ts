...
serve(async (req: Request) => {
  ...
  try {
    // ── Authenticate caller ────────────────────────────────────────
    // Accept internal service-role calls OR an authenticated user.
    // Reject anon/invalid tokens. For user callers, force organization_id
    // from THEIR users row — never trust the body value.
